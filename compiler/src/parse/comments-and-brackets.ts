...
export class CommentsAndBrackets {
      ...
      if (index < 0) {
        ...
        switch (token) {
          case StrippedToken.BlockComment:
            [parsedText, text] = this.comment.removeBlockComment(
              text,
              index,
              this.bracket.inBracketBlock
            );
            break;
          case StrippedToken.LineComment:
            [parsedText, text] = this.comment.removeLineComment(
              text,
              index,
              this.bracket.inBracketBlock
            );
          ...
          case StrippedToken.OpenBracket:
            [parsedText, text] = this.bracket.startBracketBlock(
              text,
              index,
              this.comment.inBlockComment
            );
            break;
          case StrippedToken.CloseBracket:
            [parsedText, text] = this.bracket.removeBracketBlock(
              text,
              index,
              line,
              this.comment.inBlockComment
  ...
  private nextToken(text: string): [StrippedToken, number] {
    ...
    const blockCommentIndex = Markup.findNextToken(
      text,
      StrippedToken.BlockComment
    );
    const lineCommentIndex = Markup.findNextToken(
      text,
      StrippedToken.LineComment
    );
    const openBracketIndex = Markup.findNextToken(
      text,
      StrippedToken.OpenBracket,
      requireWhitespaceBefore
    );
    const closeBracketIndex = Markup.findNextToken(
      text,
      StrippedToken.CloseBracket,
      requireWhitespaceBefore
    );
